...
async function main() {
  ...
  const previewable = [
    ...new Set(
      Object.values(props).flatMap((source) =>
        source.components.map((entry) => entry.name).filter((name) => /^[A-Z][A-Za-z0-9]*$/.test(name)),
      ),
    ),
  ].sort()
  writeFileSync(
    ...
    `/* GENERATED by scripts/generate.mjs — do not edit. */\n` +
      ...
      previewable.map((name) => `  ${name}: design.${name},`).join('\n') +
