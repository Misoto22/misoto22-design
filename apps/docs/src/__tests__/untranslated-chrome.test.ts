...
const ENGLISH_BY_DESIGN: Record<string, string> = {
  ...
  'components/HomeBand.tsx':
    'The landing band, which is inert and aria-hidden and loops by rendering itself twice. Its words are furniture giving twelve primitives something to draw — "Deploy", "Healthy", "typography" — and the components themselves are the exhibit. Everything the band shows is named and linked, in both languages, in the sections under it.',
...
describe('chrome that no mechanism can translate', () => {
  ...
  it('carries no exception for a file that has stopped needing one', () => {
    // An excuse outlives the thing it excused unless something fails when it
    // does. The test is whether the file still has English in it — not whether
    // the file still exists, which was the first version of this check and let
    // an exception survive a rewrite that had already removed every sentence
    // it was written for.
    const withProse = new Set(
      chrome()
        .filter(({ text }) => [...text.matchAll(PROSE)].length > 0)
        .map((entry) => entry.file),
    )
    expect(Object.keys(ENGLISH_BY_DESIGN).filter((file) => !withProse.has(file))).toEqual([])
  })
})
