...
export const COMPONENTS_ZH: Record<string, ComponentCopy> = {
  'bar-list': {
    name: '排行榜',
    summary: '一个排行榜——条形在名字后面，而不是旁边。',
    when: '来源排行、最慢的接口、最大的客户。横向柱状图会把三分之一的宽度花在一根重复标签的坐标轴上，而这些标签本来就可以直接写在行里。',
    accessibility: [
      '是真正的 <table>，两列、一行一个条目——因为排行榜本来就是这个结构。条形是名字那一格的背景，所以它不会变成读屏软件还要多走一遍的第二个元素。',
      'limit 会把尾巴加总成一行「Other」，而不是丢掉——一个悄悄扔掉另外四十项的「前五」是在错误陈述整体，而读者根本看不出来。',
  ...
  'big-number': {
    ...
    accessibility: [
      ...
      'value 接收的是已经格式化好的值。这个组件不去猜单位、货币或地区格式。',
    ],
  },
  'bullet-chart': {
    name: '子弹图',
    ...
    accessibility: [
      ...
      '几条带子是一个判断，却和被测量的那个数用同一种墨画出来，所以页面必须交代它们从哪来。只是把量程三等分的带子会让图看起来「已经被评估过」，其实并没有。',
  ...
  'scatter-chart': {
    ...
    accessibility: [
      ...
      '在这里承担别处由色相承担的工作的是形状。两团重叠的点，用圆形对十字形去分远比用两级灰阶好认——而且形状在叠印时仍然成立，灰阶不行。',
      '实心点自带一圈页面色描边，所以两个落在同一位置的观测仍然数得清。',
      'ZAxis 把它的度量映射到点的面积、不是半径：半径翻倍会让墨量变成四倍，这是气泡图最常见的说谎方式。',
    ],
  },
  'funnel-chart': {
    name: '漏斗图',
    summary: '只会越来越窄的若干阶段。',
    when: '注册流程、招聘管线、结账流程。当流量会分叉而不只是收窄时，那是 SankeyChart——漏斗从构造上只有一条路径。',
    accessibility: [
      'title 必填；各阶段同样会渲染成一份视觉隐藏的表格。',
      '收窄编码的是相邻两阶段之间的比例，而眼睛读的是围出来的面积，所以漏斗会放大一次平缓的下滑。组合一个 Label 把数字印上去——那就是补救。',
  ...
  'treemap-chart': {
    ...
    accessibility: [
      'title 必填。表格视图列的是叶子以及命名它的路径：一棵嵌套的树被逐行念出来，没人跟得上。',
  ...
  'box-plot': {
    ...
    when: '「这东西波动有多大」，而且要一次看六个。要看单个分布的形状，用 Histogram；观测点少到能全画出来，用 ScatterChart。',
    accessibility: [
      '一个箱子就是五个数，而五个数分不出一个峰和两个峰。双峰分布画出来的箱子，和一个中心相同的光滑分布一模一样——组件把这一点写进了自己的说明里，而不是塞在脚注里。',
  ...
  histogram: {
    name: '直方图',
    summary: '一个分布长什么形状。',
    when: '要弄懂一个分布——两个聚集、一道硬底、在超时值上堆成一摞。几个分布要并排比较，那是 BoxPlot 的活。',
    accessibility: [
      '形状是分箱宽度的属性，不只是数据的属性：同一批数切成 8 个桶和切成 80 个桶，是两张不同的图。画出这张图的规则——默认 Freedman–Diaconis——写在页面上。',
  ...
  'waterfall-chart': {
    ...
    accessibility: [
      '连接线是要小心对待的那个断言：它把各步画成一个序列，可绝大多数拆解根本不是序列——同一个月里的流失和扩张是同时发生的，而读者会把最左边那根柱子当成第一个原因。顺序是任意的时候，在 description 里说出来。',
  ...
  facet: {
    ...
    accessibility: [
      '共享取值域是默认，也是这件事的全部意义：各自独立取刻度的话，峰值 40 的一组和峰值 4,000 的一组会画出同样的形状，读者要来做的那个比较不只是丢了，是被反转了。',
