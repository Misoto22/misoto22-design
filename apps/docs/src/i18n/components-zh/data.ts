...
export const DATA_ZH: Record<string, ComponentCopyZh> = {
  ...
  'calendar-heatmap': {
    name: '日历热力图',
    summary: ['55e2386a', '一年的每日读数，画成一周一列的网格。'],
    when: ['d73f6547', '读数带日期，而问题问的是日历。Heatmap 是底下那张网格，对日期一无所知；这个组件就是所有人都会在它上面再写一遍、而且没有两次写法相同的那套排法。'],
    accessibility: [
      ['e00927c5', '这张网格是一个真正的 <table>，所以读屏软件走的结构和眼睛读的结构是同一个：一行表头、一列表头，以及一个会播报自己那条读数的单元格。'],
      ['e8453d82', '没有对应项的那一天播报的是「无数据」而不是一个数，所以对看不见这条色阶的读者来说，空缺和零是两回事。'],
      ['234e7657', '读数是按单元格取的，所以日期会和计数一起播报出来——单单行头和列头，说的只是一个星期几和一个月份，不是某一天。'],
    ],
    anatomy: [
      { hash: 'e096ffed', element: '网格', description: '一个 Heatmap：一周一列、一个星期几一行，按日期搭出来，而不是按数组顺序。每一个日期都是在 UTC 零点上做算术，所以同一个窗口在每个时区里渲染出来都一样。' },
      { hash: '285b7be8', element: '窗口', description: 'from 和 to；不传就取 values 里最早和最晚的日期。网格会往回退到第一周的开头、往后补到最后一周的结尾，所以一列永远是完整的一周。' },
      { hash: '90e00240', element: '星期几行头', description: '来自 weekdayLabels 的七个行头，在「数组」里永远是周日在先，再由 weekStartsOn 转到网格上——所以「周一开头」和「翻译」是两个各自独立的决定。' },
      { hash: '15149b8a', element: '月份列头', description: '来自 monthLabels 的名字，标在每个月碰到的第一列上，其余列留空。那些空白带着一串零宽空格，好让网格仍然分得清它们，而这正是「按列名查找」所要求的。' },
      { hash: 'ede97af2', element: '空缺', description: '窗口里没有对应项的那一天，以及首尾两个不完整的周里的每一天：一圈虚线轮廓，播报出来是「无数据」，绝不是色阶上最淡的那一格。' },
      { hash: '7a3cc7bc', element: '单元读数', description: 'describe 为那一天返回的东西，跟着这一格一起播报。它是按单元格取的，不是按数字取的，所以两天数目相同，是两条不同的读数。' },
    ],
    practices: [
      ['dfd4d08b', '只要这个窗口是关于问题、而不是关于数据的事实，就把 from 和 to 传上。靠数据推出来的话，一个安静的一月干脆就不存在了——网格悄悄变成了和旁边那张不一样的窗口，而两张照样会被拿来比。'],
      ['f43ac499', 'describe 要写成把单位叫出来的样子。一格被播报时带的行和列，是一个星期几加上一个横跨五周的月份，所以不写它，读屏软件拿到的读数就是一个日期加一个光秃秃的数。'],
      ['998655b1', '要比两张日历，就把 domain 钉死。各按自己的值域画，一个清闲的年份和一个忙碌的年份看上去一模一样，读者来做的那个对比不只是丢了，而是反了。'],
      ['f5d27636', '什么都没记录下来的那一天，不要送一个零过来。null 画成空缺，零画成色阶上最淡的那一格，而把两者画成一样，是在把一次故障变成一个清闲的时段，网格上没有任何东西说得清那到底是哪一种。'],
      ['14bd5ae2', '不要把带时间或带时区的日期交给它。每一个边界都是从 YYYY-MM-DD 按 UTC 零点解析的，所以一个在某地是 23:00 的时间戳，会落到网格里前一天上。'],
      ['c18feacd', '行不是星期几时，不要用这个组件。按小时对星期几的负载图、或者一张混淆矩阵，用的是 Heatmap，而绕道这里意味着要为本来就没有日期的格子编出日期来。'],
    ],
  },
  sparkline: {
    name: '迷你折线',
