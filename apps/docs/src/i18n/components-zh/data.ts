...
export const DATA_ZH: Record<string, ComponentCopyZh> = {
  ...
  metric: {
    name: '读数板',
    summary: ['75010bda', '一条读数，摆在一块小板上：一个标签、一个数字，以及限定它的那句话。'],
    when: ['b7ffa1b1', '控制台上一排四个。BigNumber 是同一个想法放大到标题尺寸、给「整个视图就讲这一个数字」的场合，而且要用 charts 入口；这个是那种小块，什么都不需要。'],
    accessibility: [
      ['1e2d8707', '状态点无一例外都是 aria-hidden 的，所以颜色永远不是唯一的载体——detail 里的那些字才是。'],
      ['f92af6b7', 'value 进来时就已经格式化好了。这个组件不会去猜单位、货币或地区。'],
      ['87f6f3b8', '在 asChild 下，整块板会变成调用处给的那个元素，所以那个链接或按钮就是整块板，而不是板里面的一个点击目标。'],
    ],
    anatomy: [
      { hash: '963c9024', element: '板', description: '一个 <article>，圆角取 --radius，四周一条细线，内边距 12px；用了 asChild 就是传进来的那个元素。它没有标题也没有地标：一排四个是一排读数，不是四个区块。' },
      { hash: 'af396005', element: '标签', description: 'label，等宽小字，--ink-3-aa。没有任何东西在程序上把它和那个数字绑起来，所以文档顺序就是它们之间的全部关联。' },
      { hash: '271412d7', element: '状态点', description: '设了 tone 时，标签旁边的一个 StatusDot，不闪，且 aria-hidden。它按构造就是装饰，这也正是它想说的意思必须同时写进 detail 的原因。' },
      { hash: 'f2581267', element: '数字', description: 'value，等宽字体，字号 --fs-item，表格数字，原样打印——单位、货币和地区一概不猜。用表格数字，是因为一排四个既是横着读的，也是顺着这一列往下读的。' },
      { hash: '390929c0', element: '说明', description: 'detail，在数字下面，--ink-2，小一号。一行：让这个数成为一条读数而不只是一个计数的那句话。' },
      { hash: 'cd35b728', element: '附加位', description: 'aside，放在最后，不包一层也不加样式：一条走势线、一个分母、一句提醒。和 BigNumber 的注释槽位不同，它自己不带外边距，所以一块紧凑的小板仍然紧凑。' },
    ],
    practices: [
      ['df268d49', '状态色调想说的意思，也要写进 detail。这个点按构造就是 aria-hidden 的，所以一个对故障的全部交代只是「它是红的」的小块，对读屏软件什么都没说，在灰度下也一样。'],
      ['f69f6c44', 'value 在调用处格式化好。它是被原样打印的，正是这一点让地区、货币和单位由应用决定一次，而不是被四块小板各猜一遍。'],
      ['70fa6d38', '用 asChild 时，塞进来的子元素自己不要带子节点。标签和数字是注入进它里面的，所以 `<Link href="/jobs" />` 就是全部的写法——一个自带内容的子元素，会同时拿到它自己的内容和这块板。'],
      ['b55fda46', '有一个数字、又有一个变化要报告时，不要用这个。BigNumber 有 delta、有由调用处说明的方向，还有一句会播报出来的判断；一个状态点这三样都不是，而在 aside 里把它们重新实现一遍，播报出来的东西是错的。'],
      ['c1d1727d', 'asChild 下不要往 detail 里写一整句话。整块板都在一个链接里面，所以这个链接的可访问名称是里面每一个词按顺序读一遍，多加的一个从句，就是每次走到这个链接都要多听一遍的从句。'],
      ['9d3fee45', '不要在里面放标题。这块板是一个没有名字的 <article>，所以放进去的标题会开出一个读屏软件会列出来的区块——四块小板于是读起来是页面的四个区块，而不是一排数字。'],
    ],
  },
  'big-number': {
    name: '大数字',
