...
const LANES = [
  { name: 'Platform', hue: 'var(--series-1)', commits: 2462, opened: 2078, merged: 2003 },
  { name: 'Side projects', hue: 'var(--series-2)', commits: 2406, opened: 609, merged: 563 },
]
...
const DAYS_SERIES = { contributions: { label: 'Contributions' } } satisfies ChartConfig
...
const MEMBERS = [
  ...
  { lane: 'Side projects', login: 'aoi', commits: 2406, opened: 609, merged: 563 },
]
...
function Tile({
  ...
  return (
    <Card style={{ gridArea: area }} className="flex min-h-0 flex-col">
      ...
      <CardBody className="flex min-h-0 flex-1 flex-col p-0">{children}</CardBody>
    </Card>
  )
}
...
export function ActivityBoard() {
  ...
  const order = (rows: typeof MEMBERS) =>
    [...rows].sort((a, b) => {
      ...
      return flip * (Number(b[sort.key as 'commits']) - Number(a[sort.key as 'commits']))
    })

  return (
    <div
      className="grid min-h-[42rem] gap-2 @container"
      style={{
        gridTemplateColumns: 'repeat(12, minmax(0, 1fr))',
        gridTemplateRows: 'auto auto minmax(0, 1.15fr) minmax(0, 1fr)',
        gridTemplateAreas: [
          '"head head head head head head head head head head head head"',
          '"kpi kpi kpi kpi kpi kpi kpi kpi alert alert alert alert"',
          '"trend trend trend trend trend days days days alert alert alert alert"',
          '"team team team team spend spend spend spend alert alert alert alert"',
        ].join(' '),
      }}
    >
      <header
        style={{ gridArea: 'head' }}
        className="relative flex flex-wrap items-center gap-x-3 gap-y-2 border-b-2 border-(--ink) pb-2"
      >
        <h2 className="font-heading text-base font-semibold whitespace-nowrap">Activity</h2>
        ...
        {/* Indeterminate on purpose: nothing here knows a percentage. It rides
            ...
            appears. */}
        <Progress label="Loading the window" className="absolute inset-x-0 -bottom-0.5 h-0.5" />
      </header>

      <div
        style={{ gridArea: 'kpi', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))' }}
      ...
      <Tile area="alert" title="Needs attention" note="23 stalled · oldest 124d">
        <div className="scroll-slim flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-3 pb-2">
          ...
          <Alert tone="warning" title="Idle 124 days: core primitives and scaffolding">
            harbour#96
          </Alert>
        ...
        <div className="shrink-0 border-t border-(--rule) px-3 py-2">
          <BarList label="Stalled requests by age" showLabel sort={false} items={AGES} />
        </div>
      </Tile>

      <Tile area="trend" title="Commits over the window" note="6 buckets · 08-09 → 09-03">
      ...
      <Tile area="days" title="My days" note="1,984 contributions · peak 09-04">
        <div className="flex min-h-0 flex-1 px-2 pb-2">
          <BarChart
            config={DAYS_SERIES}
            data={DAYS}
      ...
      <Tile area="team" title="By member" note="since 08-09">
          ...
          >
            <THead>
              <TR>
                <TH
                  sortable
                  sortDirection={sort.key === 'login' ? sort.direction : 'none'}
                  onSort={() => onSort('login')}
                >
                  Member
                </TH>
            ...
            {LANES.map((lane) => (
              <TBody key={lane.name}>
                ...
                {order(MEMBERS.filter((member) => member.lane === lane.name)).map((member) => (
                  <TR key={`${lane.name}/${member.login}`}>
                    <TD>
                      <span className="whitespace-nowrap">{member.login}</span>
                    </TD>
                    {COLUMNS.map((column) => (
      ...
      <Tile area="spend" title="Spend per day" note="all activity — not attributable to a lane">
        <div className="flex min-h-0 flex-1 px-2 pb-2">
          <BarChart
            config={SPEND_SERIES}
            data={SPEND}
