  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-10">
      <PageIntro
        eyebrow="Start"
        title="Principles"
        summary="Eight rules, each written so that a specific piece of work can be shown to break it. A principle that only says what to aim for settles no argument."
        crumbs={[{ label: 'Principles' }]}
      />
        ...
        {LAWS.map((law) => (
          <article key={law.n} className="grid gap-4 py-8 md:grid-cols-[3rem_minmax(0,1fr)]">
            ...
            <div className="flex flex-col gap-3">
              ...
              <p className="m-0 max-w-(--w-reading) border-l border-(--rule-2) pl-4 text-[13px] leading-relaxed text-(--ink-3-aa)">
                <span className="eyebrow">Rules out</span>
                <br />
                {law.rules_out}
              </p>
            </div>
