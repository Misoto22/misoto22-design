...
export function Checkbox({ className, ...props }: CheckboxProps) {
  return (
    <CheckboxPrimitive.Root
      className={cn(
        'inline-flex size-[18px] shrink-0 items-center justify-center rounded-(--radius-sm) border border-(--rule-2) bg-(--paper) text-(--paper) transition-colors duration-(--duration-fast) data-[state=checked]:border-(--ink) data-[state=checked]:bg-(--ink) data-[state=indeterminate]:border-(--ink) data-[state=indeterminate]:bg-(--ink) disabled:opacity-(--disabled-opacity) disabled:pointer-events-none',
        className,
      )}
      {...props}
    >
