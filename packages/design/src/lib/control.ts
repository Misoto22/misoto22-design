...
export const CONTROL_BASE =
  'w-full rounded-(--radius) border bg-(--paper) px-3.5 py-2.5 text-sm text-(--ink) transition-colors duration-(--duration-fast) placeholder:text-(--ink-3-aa) hover:border-(--rule-hard) focus:border-(--ink) disabled:opacity-(--disabled-opacity) disabled:pointer-events-none'
